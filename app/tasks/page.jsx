...
import EditCMC from '../components/EditTaskForm.jsx'
import EditTask from '../components/EditTaskForm.jsx'
...
export default function TaskPage() {
    ...
    const columnDefs = [
        ...
        { headerName: "Action Required", field: "action_required", width: 180 },
        ...
        { headerName: "Days to Remind", field: "days_to_remind", width: 150 },
...

        {
            ...
            width: 150,
            renderCell: (params) => {
                const { task_date, days_to_remind } = params.row;
                if (!task_date || !days_to_remind) return '';

                const baseDate = new Date(task_date);


                const nextServiceDate = new Date(baseDate.getTime() + days_to_remind * 24 * 60 * 60 * 1000);
...

        {
            ...
            renderCell: (params) => {
                ...
                const baseDate = new Date(task_date); // when task started
                baseDate.setHours(0, 0, 0, 0);

                // due date = task_date + days_to_remind
                const dueDate = new Date(baseDate.getTime() + days_to_remind * 24 * 60 * 60 * 1000);
